...
namespace B22_Ex05_ItayGrinberg_209413277_GuyGanot_207044363
{
    public class Board
    {
        ...
        public bool ValidateInput(string i_MoveToMake, Player i_CurrentPlayer, Tile i_LastMoveTile, bool i_IsSecondTurn = false)
        {
            //[Af>Bf] => [A,f,B,f] A<= X <=F 
            bool isValid = false;
            if (i_MoveToMake.Length == 5 && i_MoveToMake[2] == '>')
            {
                    ...
                    isInBound(destinationRow, destinationColumn))
                {
                    ...
                    {
                        if ((i_LastMoveTile == null && isOrientationOkay) || i_IsSecondTurn || i_LastMoveTile == BoardMatrix[originRow, originColumn] || !isASimplePuck)
                        {
            ...
            if (!doesAlternativeExists)
            {
                foreach (Tile queen in i_CurrentPlayer.QueenPucks.OrderBy(queen => r.Next()))
                {
                    ...
                    for (int i = 0; i < directions.GetLength(0); i++)
                    {
                        ...
                        {
                            MovementIndexString = IndexToCharArray(queen.ColumnNumber, queen.RowNumber,
                                colNumber += directions[i, 1], rowNumber += directions[i, 0]);
                            break;
                        }
                        else
                        {
        ...
        public int[] GetCoordinate(string i_TargetTile)
        {
            const char rowsIndicator = 'a';
            const char columnIndicator = 'A';
            //Convert characters to index
            int destinationColumn = i_TargetTile[0] - columnIndicator;
            int destinationRow = i_TargetTile[1] - rowsIndicator;
            return new[] { destinationColumn, destinationRow };
        }

        public string IndexToCharArray(int i_OriginColumn, int i_OriginRow, int i_DestinationColumn, int i_DestinationRow)
        {
            const char rowsIndicator = 'a';
            const char columnIndicator = 'A';
        ...
        public bool FoundEatInPath(int i_OriginRow, int i_OriginColumn, int i_RowOrientation, int i_ColOrientation, Player i_Player)
        {
            ...
            do
            {
                ...
                else if (oneStepPuck != null && oneStepPuck.Puck != ' ')
                {
                    int prevRow, prevCol;
                    //plus or minus
                    if (isInBound(prevRow = destinationRow + i_RowOrientation, prevCol = destinationCol + i_ColOrientation))
                    {
                        Tile prevTile = BoardMatrix[prevRow, prevCol];
                        Tile prevPrevTile = null;
                        if (isInBound(prevRow -= i_RowOrientation, prevCol -= i_ColOrientation))
                        {
                            prevPrevTile = BoardMatrix[prevRow, prevCol];
                        }
                        if (prevTile.Puck != ' ' && prevPrevTile != null)
                        {
                            break;
                        }
        ...
        public static bool DoesPuckBelongToPlayer(Player i_Player, Tile location)
        {
